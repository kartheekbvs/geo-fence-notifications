...
import { getDistanceInMeters, getAddressFromCoordinates } from './services/geolocationService';

declare const google: any;
declare const emailjs: any;
const CLIENT_ID = '930575952017-crls3493s43tld7rmsr0unf1il7qi627.apps.googleusercontent.com';
const EMAILJS_PUBLIC_KEY = "3BMbctG8PLkgdT2XV";
const EMAILJS_SERVICE_ID = "service_i8b6n69";
const EMAILJS_TEMPLATE_ID = "template_g2gp10s";
...
const App: React.FC = () => {
    ...
    const sendDangerNotification = useCallback(async (recipientEmail: string): Promise<boolean> => {
        if (!user || !location) {
            alert("User or location data is not available to send a notification.");
            return false;
        }
        ...
        try {
            ...
            const templateParams = {
                ...
                loc: `https://www.google.com/maps?q=${location.latitude},${location.longitude}`,
                adress: address,
            };
            ...
            console.error("Failed to send danger notification:", error);
            return false;
        }
    }, [user, location]);
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen text-gray-100 font-sans">
            <main className="flex-grow h-1/2 md:h-full">
                {memoizedMap}
            </main>
            <aside className="w-full md:w-96 lg:w-1/3 xl:w-1/4 h-1/2 md:h-full flex flex-col bg-black/20 backdrop-blur-md border-l border-white/20 shadow-lg">
                <ControlPanel 
                    user={user}
                    onSignOut={handleSignOut}
                    isTracking={isTracking}
                    onToggleTracking={() => isTracking ? stopTracking() : startTracking()}
                    onSendDanger={sendDangerNotification}
                    onAddFence={addFence}
